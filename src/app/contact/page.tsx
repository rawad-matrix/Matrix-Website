    ...
    <>
      ...
      <section className="bg-[#F8F9FB] py-[110px] max-[768px]:py-[72px]">
        <div className="max-w-[1280px] mx-auto px-8 max-[640px]:px-5">
          <div className="grid grid-cols-[1fr_1.5fr] gap-10 items-start max-[900px]:grid-cols-1">
            {/* Left: contact card */}
            <div
              className="rounded-[2px] p-10 text-white sticky top-28"
            ...
            >
              ...
              <div className="flex flex-col gap-5 mb-8">
                {[
                  ...
                  <div key={label} className="flex gap-4 items-start">
                    <div className="flex-shrink-0 w-9 h-9 grid place-items-center bg-[rgba(27,111,204,.15)] rounded-[2px] text-[#1B6FCC]">
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">{icon}</svg>
                    </div>
                    {href ? (
                      <a href={href} className="font-dm text-[14px] text-white/80 hover:text-white transition-colors mt-[9px]">{label}</a>
                    ) : (
                      <span className="font-dm text-[14px] text-white/80 mt-[9px]">{label}</span>
                    )}
                  </div>
                ))}
              </div>
              ...
              <a
                ...
                className="flex items-center justify-center gap-3 w-full bg-[#25D366] text-white px-6 py-4 font-dm font-semibold text-[13.5px] uppercase tracking-[0.04em] rounded-[2px] hover:brightness-110 transition-all duration-150 mb-6"
              >
            ...
            {/* Right: form */}
            <div className="bg-white rounded-[2px] p-10" style={{ border: '1px solid #E2E8F0' }}>
              <h2 className="font-barlow font-bold text-[32px] uppercase text-[#1F2330] mb-6">Send a Message</h2>
              <ContactForm />
            </div>
          </div>
        </div>
      ...
      {/* Maps embed */}
      <div className="overflow-hidden" style={{ borderRadius: '2px', height: '400px' }}>
        <iframe
          src="https://maps.google.com/maps?q=MUC+University+Building,+Khaldeh+Roundabout,+Beirut,+Lebanon&t=&z=17&ie=UTF8&iwloc=&output=embed"
          width="100%"
          height="400"
          style={{ border: 0 }}
          allowFullScreen
